...
//SMB 1 font by igorsou3000
#include "font_smb1map.h"
s32 Font_SMB1_GetWidth(struct FontData *this, const char *text)
{
	(void)this;
...
void Font_SMB1_DrawCol(struct FontData *this, const char *text, s32 x, s32 y, FontAlign align, u8 r, u8 g, u8 b)
{
	//Offset position based off alignment
	switch (align)
	{
	...
	while ((c = *text++) != '\0')
	{
		...
		//Draw character
		RECT src = {font_smb1map[c].charX, font_smb1map[c].charY, font_smb1map[c].charW, font_smb1map[c].charL};
		Gfx_BlitTexCol(&this->tex, &src, x, y, r, g, b);
