...
{
	//the og resolution of da mod it's 160x240, so i * by 2 for make be psx resolution
	//Draw character
	fixed_t x = this->x - FIXED_MUL(stage.camera.x, parallax) - FIXED_DEC(cframe->off[0] * 2,1);
	fixed_t y = this->y - FIXED_MUL(stage.camera.y, parallax) - FIXED_DEC(cframe->off[1] * 2,1);
